...
export function AppShell({ children, className }: { children: React.ReactNode; className?: string }) {
  ...
  React.useEffect(() => {
    if (!loading && !user) {
      void navigate({ to: "/signin", search: { redirect: `${location.pathname}${location.search}` } });
    }
  }, [loading, user, navigate, location.pathname, location.search]);

  if (loading || !user) {
    return <div className="bg-aurora grid min-h-screen place-items-center text-sm text-muted-foreground">Checking your session...</div>;
  }
