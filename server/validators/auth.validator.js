...
export function updateProfileValidator({ body }) {
  ...
  if (!isRequestBody(body)) {
    return {
      valid: false,
      errors: [
        { field: "profile", message: "Profile updates must be an object" },
      ],
    };
  }
  ...
  if (
    ...
    (typeof body.avatar !== "string" || body.avatar.length > 2048)
  ) {
    errors.push({
      field: "avatar",
      message: "Avatar must be a URL of 2,048 characters or fewer",
    });
  }
  if (
    ...
    (!Array.isArray(body.discoveryLocations) ||
      ...
      body.discoveryLocations.some(
        (loc) => typeof loc !== "string" || loc.length > 120,
      ))
  ) {
    errors.push({
      field: "discoveryLocations",
      message: "Up to 3 valid location names required",
    });
  }
  return { valid: errors.length === 0, errors };
}
...
export function updateInterestsValidator({ body }) {
  ...
  if (!isRequestBody(body)) {
    return {
      ...
      errors: [
        {
          field: "interests",
          message: "Interests must be provided as an object field",
        },
      ],
    };
  }
  if (Object.keys(body).some((field) => field !== "interests")) {
    errors.push({
      field: "interests",
      message: "Only interests can be updated here",
    });
  }
  if (!("interests" in body)) {
    ...
    errors.push({
      field: "interests",
      message:
        "Interests must be unique values from the TrendGo interest categories",
    });
  }
