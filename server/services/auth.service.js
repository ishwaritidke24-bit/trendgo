...
export async function signup({ name, email, password, location = "", interests = [], avatar = "" }) {
  const normalizedEmail = normalizeEmail(email);
  const existingUser = await User.findOne({ email: normalizedEmail }).lean();
  if (existingUser) throw createHttpError(409, "An account with that email already exists", "EMAIL_IN_USE");

  const passwordHash = await bcrypt.hash(password, BCRYPT_ROUNDS);
  const user = await User.create({ name: name.trim(), email: normalizedEmail, passwordHash, location, interests, avatar });
...
export async function signin({ email, password }) {
  ...
  if (!validPassword) throw createHttpError(401, "Invalid email or password", "INVALID_CREDENTIALS");
